...
def generate_hashes(
    ...
    peaks_sorted: list[Peak] = sorted(peaks, key=lambda x: x[1])
    ...
    for i, anchor in enumerate(peaks_sorted):
        ...
        for j in range(i + 1, len(peaks_sorted)):
            ...
            hash_int = create_hash(int(anchor_freq), int(target_freq), int(time_delta))
...
def create_hash(freq1: int, freq2: int, time_delta: int) -> str:
